  ...
  body.people.forEach(async (element) => {
    if(element.isAttending)
      if(
        ...
        (element.covidStatus !== 'Will Be Vaccinated' &&
        element.covidStatus !== 'Will Be Tested')
      )
        throw new Error('Invalid body.');
...
async function saveSingleResponse(saveObj) {
  const query = 'update rsvp_person ' + 
    'set ' +
    (saveObj.isAttending ? `meal_id = ${saveObj.mealID}, ` : '') +
    (saveObj.isAttending ? `covid_status = '${saveObj.covidStatus}', ` : '') +
    (saveObj.isAttending ? (saveObj.allergies ? `allergy = '${saveObj.allergies}', ` : '') : '') +
